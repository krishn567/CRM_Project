...
const Header = () => {
    ...
    useEffect(() => {
        ...
        const fetchAllWorkspaces = async () => {
            ...
            try {
                ...
                const result = await response.json();                
                console.log("API Response:", result);
                // setWorkspacedata((prev) => ({ ...prev, ...result.data }));
                setWorkspacedata(result.data);
                console.log("this is data---",workspacedata)
            } catch (error) {
                console.log("Error fetching workspaces:", error);
            }
        };
    ...
    const handleSelectChange = (e) => {
        ...
        if (selectedId) {
            // localStorage.setItem("finalworkspace", selectedId)
            localStorage.setItem("workspace_id", selectedId); 
            navigate(`/workspacedetail/${selectedId}`);   
        }
    };
            ...
            <div className="workspaces relative w-[10vw]">
                <form className="max-w-sm mx-auto">
                    <select
                        ...
                        onChange={handleSelectChange} 
                    >
                        {/* <option value="">Select Workspace</option> Default option */}
                        {workspacedata.map((element) => (
                            <option key={element.workspace_id} value={element.workspace_id}>
                                {element.workspace_name}
                            </option>
...
//           localStorage.setItem("workspace_logo", base64String); // Store in localStorage
//           setFormValues({ ...formValues, [name]: base64String }); // Store in form values
//         };
